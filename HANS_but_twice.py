...
max_length = 0
start_length = 3
block_colours = [(255, 255, 255), (210, 0, 0), (255, 255, 0), (200, 200, 200), (0, 0, 210)]
length = 3
last_right = False
...
def display_value():
    ...
    iteration_text_one = screen_font.render(f"H.A.N.S. #1 (Red) Length: {snake_one.length}", True, (0, 0, 0))
    iteration_text_two = screen_font.render(f"H.A.N.S. #2 (Blue) Length: {snake_two.length}", True, (0, 0, 0))
    screen.blit(iteration_text_one, (screen.get_width() - 250, 0))
...
while not done:
    ...
    if not start:
        snake_one = Snek([[10, 10]], randElement(directions), ai=ai_bool)
        snake_two = Snek([[30, 30]], randElement(directions), ai=ai_bool)
