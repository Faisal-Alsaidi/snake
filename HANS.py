...
iteration = 1
max_length = 0
block_colours = [(255, 255, 255), (0, 0, 0), (255, 255, 0), (200, 200, 200), (69, 69, 255)]
length = 2
last_right = False
...
def checkPath(snek_pos, goal_pos):
    ...
    for i in range(goal_pos[0], snek_pos[0][0], increment_x):
        if map.map[i][snek_pos[0][1]] in (1, 3):
            return False
    for i in range(goal_pos[1], snek_pos[0][1], increment_y):
        if map.map[goal_pos[0]][i] in (1, 3):
            return False
    return True
...
def checkDirection(snek_pos, direction):
    ...
    y = snek_pos[0][1]
    if direction == "LEFT":
        if map.map[x-1][y] in (1, 3):
            return False
    elif direction == "RIGHT":
        if map.map[x+1][y] in (1, 3):
            return False
    elif direction == "UP":
        if map.map[x][y-1] in (1, 3):
            return False
    elif direction == "DOWN":
        if map.map[x][y+1] in (1, 3):
            return False
